...
function Like($pat, $dbid = 0) {
	$dbtype = GetConnectionType($dbid);
	if ($dbtype == "POSTGRESQL") {
		return (Config("USE_ILIKE_FOR_POSTGRESQL") ? " ILIKE " : " LIKE ") . $pat;
	} elseif ($dbtype == "MYSQL") {
		if (Config("LIKE_COLLATION_FOR_MYSQL") != "") {
			return " LIKE " . $pat . " COLLATE " . Config("LIKE_COLLATION_FOR_MYSQL");
		} else {
			return " LIKE " . $pat;
		}
	} elseif ($dbtype == "MSSQL") {
		if (Config("LIKE_COLLATION_FOR_MSSQL") != "") {
			return " COLLATE " . Config("LIKE_COLLATION_FOR_MSSQL") . " LIKE " . $pat;
		} else {
			return " LIKE " . $pat;
		}
	} else {
...
function GetDebugMessage() {
	...
	<button type="button" class="btn btn-tool" data-widget="collapse"><i class="fas fa-minus"></i></button></div></div><div class="card-body">' . $msg . '</div></div>' : "";
}
