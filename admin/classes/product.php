...
class product extends DbTable
{
	...
	public function __construct()
	{
		...
		// code
		$this->code = new DbField('product', 'product', 'x_code', 'code', '`code`', '`code`', 200, 50, -1, FALSE, '`code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// amountLeft
		$this->amountLeft = new DbField('product', 'product', 'x_amountLeft', 'amountLeft', '`amountLeft`', '`amountLeft`', 3, 11, -1, FALSE, '`amountLeft`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->amountLeft->Nullable = FALSE; // NOT NULL field
		$this->amountLeft->Required = TRUE; // Required field
		$this->amountLeft->Sortable = TRUE; // Allow sort
		$this->amountLeft->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['amountLeft'] = &$this->amountLeft;

		// price
		$this->price = new DbField('product', 'product', 'x_price', 'price', '`price`', '`price`', 3, 11, -1, FALSE, '`price`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// supermarket
		$this->supermarket = new DbField('product', 'product', 'x_supermarket', 'supermarket', '`supermarket`', '`supermarket`', 3, 11, -1, FALSE, '`supermarket`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->supermarket->Nullable = FALSE; // NOT NULL field
		$this->supermarket->Required = TRUE; // Required field
		$this->supermarket->Sortable = TRUE; // Allow sort
		$this->supermarket->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['supermarket'] = &$this->supermarket;

		// picture
		$this->picture = new DbField('product', 'product', 'x_picture', 'picture', '`picture`', '`picture`', 200, 255, -1, TRUE, '`picture`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->amountLeft->DbValue = $row['amountLeft'];
		$this->price->DbValue = $row['price'];
		$this->supermarket->DbValue = $row['supermarket'];
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->amountLeft->setDbValue($rs->fields('amountLeft'));
		$this->price->setDbValue($rs->fields('price'));
		$this->supermarket->setDbValue($rs->fields('supermarket'));
	...
	public function renderEditRow()
	{
		...
		$this->amountLeft->EditValue = $this->amountLeft->CurrentValue;
		...
		$this->supermarket->EditValue = $this->supermarket->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->amountLeft);
					$doc->exportCaption($this->price);
					$doc->exportCaption($this->supermarket);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->amountLeft);
						$doc->exportField($this->price);
						$doc->exportField($this->supermarket);
